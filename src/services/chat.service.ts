...
export async function getOrCreateChat(opts: { caseId?: string; userA: string; userB: string }) {
...
export async function isParticipant(chatId: string, userId: string) {
  ...
  if (chat.case) {
    if (chat.case.clientId === userId || chat.case.lawyerId === userId) return true;
  }
...
export async function ensureParticipantsShareRelation(userA: string, userB: string) {
...
export async function addMessage(chatId: string, senderId: string, text?: string, attachments?: string[]) {
...
export async function getParticipants(chatId: string) {
...
export async function markMessageRead(messageId: string, readerId: string) {
