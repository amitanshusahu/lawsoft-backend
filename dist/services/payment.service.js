...
export async function createOrder(opts) {
    ...
    if (razorClient) {
        ...
        const order = await razorClient.orders.create({
        ...
        await prisma.payment.update({ where: { id: payment.id }, data: { metadata: { ...(payment.metadata || {}), providerOrderId: order.id, providerOrder: order } } });
...
export async function verifyWebhook(bodyString, signatureHeader) {
    const secret = config.payments?.razorpay?.keySecret;
    if (!secret)
