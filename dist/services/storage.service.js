 ...
 */
const client = new S3Client({ endpoint: config.aws.s3.endpoint, region: config.aws.s3.region, credentials: { accessKeyId: config.aws.s3.accessKeyId, secretAccessKey: config.aws.s3.secretAccessKey } });
// try{
//     client.send(new CreateBucketCommand({Bucket:config.aws.s3.bucket}));
// }catch(err : any){
//     console.error("Error in S3 Client Creation:", err.name);
// }
export async function createPresignedUpload(key, contentType = 'application/octet-stream', expiresInSec = 3600) {
